...
public class Select {
	...
	public static void select2() {
		...
		while(srun2) {
			...
			System.out.println("| 3. 상세내용으로 조회  | 4. 조회화면으로          |");
			System.out.println("====================================================");
			System.out.print("사용할 메뉴의 번호를 입력 >> ");
	...
	// 총 수입 ###,### 타입
	public static String inamount1() {
		String sql = "SELECT sum(in_amount) as amount FROM income";
		DecimalFormat df = new DecimalFormat("###,###");
		...
		String amount1 = df.format(amounti);
	...
	// 총 지출 ###,### 타입
	public static String outamount1() {
		String sql = "SELECT sum(out_amount) as amount FROM expenditure";
		DecimalFormat df = new DecimalFormat("###,###");
		...
		String amount2 = df.format(amounte);
	...
	public static String bamount() {
		...
		String bamount1 = df.format(bamount);
	...
	public static InquiryVo selectAlli() {
		...
		try {
			...
			System.out.println("===============================================================");
			System.out.println(" 총 수입 금액: \t"+inamount1()+" 원");
			System.out.println("===============================================================");
	...
	public static InquiryVo selectAlle() {
		...
		try {
			...
			System.out.println("===============================================================");
			System.out.println(" 총 지출 금액: \t"+outamount1()+" 원");
			System.out.println("===============================================================");
	...
	public static InquiryVo selectInD() {
		...
		try {
			...
			System.out.println("\n예외 발생시 처라할 코드: 쿼리문 조회(수입 날짜(년/월)내용)");
		}
	...
	// 수입 날짜(일) 조회
	public static InquiryVo selectInDD() {
		InquiryVo selecti3 = new InquiryVo();
		Scanner sc = new Scanner(System.in);
		
        System.out.println("\n===================================");
        System.out.println(" 찾고싶은 날짜(일)를 입력하세요 ");
        System.out.println(" ex) 찾고 싶은 날짜 = ?/?/? ");
        System.out.println("===================================");
        System.out.print("날짜를 입력 >> ");
        selecti3.setIn_date(sc.nextLine());
		...
		try {
			String sql = "SELECT to_char(in_date, 'YYYY-MM-DD') AS in_date,in_list,in_amount,"
					+ "in_memo FROM income WHERE in_date=? ORDER BY in_date";
			
			conn = DBManager.getCoonnection();			// DB 연결
			pstmt = conn.prepareStatement(sql);			// 쿼리문 실행
			pstmt.setString(1, selecti3.getIn_date());	// 날짜(일) 입력 값
			...
			while(rs.next()) {
				System.out.println(rs.getString("in_date")+"\t"+rs.getString("in_list")
					+"\t\t"+rs.getInt("in_amount")+"\t\t"+rs.getString("in_memo"));
				
			}
		} catch(Exception e) {
			System.out.println("\n예외 발생시 처라할 코드: 쿼리문 조회(수입 날짜(일)내용)");
		}
		DBManager.close(conn, pstmt, rs);	// DB 닫기
		return selecti3;
	}
	...
	public static InquiryVo selectOutD() {
		...
		try {
			...
			conn = DBManager.getCoonnection();					// DB 연결
			pstmt = conn.prepareStatement(sql);					// 쿼리문 실행
			pstmt.setString(1, "%"+selecte2.getOut_date()+"%");	// 날짜(년/월) 입력 값
			...
			System.out.println("\n예외 발생시 처라할 코드: 쿼리문 조회(지출 날짜(년/월)내용)");
		}
	...
	// 지출 날짜(일) 조회
	public static InquiryVo selectOutDD() {
		InquiryVo selecte3 = new InquiryVo();
		Scanner sc = new Scanner(System.in);
        ...
        System.out.println("===================================");
        System.out.print("날짜를 입력 >> ");
        selecte3.setOut_date(sc.nextLine());
		...
		try {
			String sql = "SELECT to_char(out_date, 'YYYY-MM-DD') AS out_date,out_list,out_amount,"
					+ "out_memo FROM expenditure WHERE out_date=? ORDER BY out_date";
			
			conn = DBManager.getCoonnection();				// DB 연결
			pstmt = conn.prepareStatement(sql);				// 쿼리문 실행
			pstmt.setString(1, selecte3.getOut_date());	// 날짜(일) 입력 값
			...
			while(rs.next()) {
				System.out.println(rs.getString("out_date")+"\t"+rs.getString("out_list")+
						"\t\t"+rs.getInt("out_amount")+"\t\t"+rs.getString("out_memo"));
